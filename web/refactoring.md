...
- Flag **large switch/if chains that branch on object type or discriminant** — candidates for replacing with polymorphism (a shared interface, each variant its own class or module). When naming the refactoring move, give it a pattern target: conditional branching on type to select behavior converges on **Strategy** (inject the varying behavior via an interface); branching driven by state transitions converges on **State** (each state as its own type); a notification/event-listener structure converges on **Observer**; a class that needs responsibilities layered on dynamically converges on **Decorator**. Naming the pattern target shares vocabulary with reviewers and gives the refactoring a concrete shape to implement toward — a "Replace Conditional with Polymorphism" move without a target is harder to scope, review, and finish
- Flag **complex async functions sharing extensive local state across sub-steps**: when a server action, service function, or route handler performs multiple phases that accumulate and thread state between them via a growing list of local `const`s, extract the whole function into a dedicated class or closure-based module — its parameters become constructor arguments or factory inputs, its shared intermediate state becomes instance fields or closure variables, and each phase becomes a separate named method. This move (the **Method Object** pattern) differs from grouping parameters: it gives each sub-step a place to share state directly through the object and makes each phase independently testable
...
- **Write fitness functions to enforce architectural boundaries in CI.** A fitness function is an automated test that fails when a forbidden cross-boundary import is introduced — not a rule enforced by review, but a check that runs every build and cannot be skipped under time pressure. For Next.js/TypeScript: configure `dependency-cruiser` with rules that reject any import from a `sales/` module into a `warehouse/` module, or from any client component into a server-only module; the `depcruise --validate` command exits non-zero in CI when a rule is violated. Where `dependency-cruiser` is not yet set up, `eslint-plugin-import/no-restricted-paths` or a custom script that greps import statements and exits non-zero achieves the same intent. The cost is one configuration entry per boundary; the return is automated enforcement for the life of the repo — a failing CI check catches the violation before it compounds into multiple layers of illegal coupling.

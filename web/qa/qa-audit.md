...
Consult `web/resources.md` as needed — it lists the authoritative testing frameworks (Vitest, Playwright, RTL, MSW), the official Next.js and React docs for framework behavior, and the OWASP sources for security-relevant test gaps.
...
- **Audit for a stable testing seam below the UI.** Business rules should be verifiable through a framework-free unit — a service function, a plain validation, an entitlement resolver — without standing up the web server, database, or browser. Tests that can only reach business logic through the E2E layer are fragile and expensive: any markup change breaks them, and the team learns to fear UI refactoring. Audit whether each critical rule (authorization, entitlement decisions, data transformations, error-path branching) is accessible through such a seam. If not, the missing seam — not the missing E2E test — is the architectural gap. Flag it as requiring an "Extract to plain function" move that converts a Playwright-only behavior into a fast, stable unit test. A testing seam that lets you bypass the server/UI layer, inject test state, and exercise business rules directly is the highest-leverage architectural investment a low-coverage codebase can make.
...
- Audit for **N+1 query patterns**: ORM code that fetches N records and then accesses a relation on each item in a loop, issuing one additional query per item instead of a single join or eager-load. These are invisible at dev-data scale and become full-table-scan-level bottlenecks in production. Flag any `include`/`select` relationship traversal that resolves lazily, any `findById`/`findFirst` call inside a loop over a result set, and any route handler whose DB call count grows linearly with the result size. The fix is a joining query or an explicit batch fetch; the test is to assert query count (or query time) against a seed of realistic size, not a handful of rows
...
- Note the natural divergence between tests and production code over time: tests tend to become more concrete and specific as the team learns what can go wrong; production code tends to become more abstract and general as it is refactored. Structural coupling blocks this healthy divergence — a test that imports and calls concrete implementation modules directly must be updated every time those modules are refactored, even when behavior is unchanged. A **testing API** or stable behavioral seam (service function signatures, plain validation modules, entitlement resolvers) that hides the production code's internal structure from tests allows both sides to evolve independently: internal modules can be renamed, extracted, or reorganized without touching a passing test.
...
- Apply the **F.I.R.S.T.** checklist to the suite as a whole: **Fast** — tests that are slow discourage frequent running; flag any suite where the full loop exceeds ~10 minutes, identify the bottleneck (real-DB access and E2E tests are the usual culprits), and push behavior down to faster tiers or move expensive suites to a scheduled run; **Independent** — already covered above (inter-dependent tests, shared state); **Repeatable** in any environment — no dependency on a live third-party API, a real system clock, or environment-specific config; **Self-Validating** — tests must produce a binary pass/fail without manual log inspection or file diffing (a test that "passes" by timing out or requires reading output to interpret is not self-validating); **Timely** — tests written alongside the code they cover, not after; a large route handler or service added without any accompanying tests signals code that was never designed for testability, which manifests as logic fused into framework boundaries and missing plain-function seams.
- **Validate suite strength with mutation testing**, not just coverage numbers. Mutation testing tools automatically introduce small plausible code changes — flip a `>` to `>=`, change `+` to `-`, delete a `return` statement — and check whether the test suite detects each change. A line that executes in a test proves only that it ran; a mutation that survives means no assertion would fail if that code changed, exposing tests that verify execution rather than correctness. For TypeScript, `Stryker` is the standard mutation testing tool with Vitest and Jest integration. Apply it to high-risk areas first — auth, payment flows, validation logic, schema boundaries — rather than the whole codebase; the signal is sharpest where defects are most costly. When a surviving mutation is found, add a test that catches it before fixing the mutation; this turns the audit finding directly into a regression test.

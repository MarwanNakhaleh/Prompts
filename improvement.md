...
With the remaining context, start reading "current_book" beginning at position "current_position" (line number) from the JSON file. Read in chunks of ~1500 lines to avoid hitting context limits. Read the book in full, INCLUDING all appendices — do not skip appendix material. The only sections you may skip are the index and the glossary (they carry no conceptual content); raw code listings may be skimmed rather than read line by line.

As you read (the main text, footnotes, endnotes, bibliography, "notes," or "further reading"), note any OTHER books the author references, cites, or recommends. Record each in `knowledge-base/book-list.json` — a JSON array of objects, each with `name` (the referenced book's title) and `author`. Only record a reference when BOTH the title AND its author are explicitly stated; if either is missing, ignore that reference. Before writing, read the existing `knowledge-base/book-list.json` (create it as `[]` if it does not exist), append only genuinely new entries, and dedupe by title so you never overwrite or duplicate prior entries.

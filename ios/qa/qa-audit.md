...
- Audit for **N+1 fetch patterns**: Core Data or SwiftData code that fetches N objects and then accesses a relationship property on each item in a loop, issuing N additional SQL statements instead of one join-based fetch. Flag relationship property accesses inside loops over fetch results, and use of `NSFetchRequest` without relationship key paths or `relationshipKeyPathsForPrefetching` where relations are consumed. The fix is a prefetch request or a single batch fetch for the related objects; the test is to assert fetch count against a store seeded to realistic size, not a handful of fabricated rows — the bug is invisible with small data

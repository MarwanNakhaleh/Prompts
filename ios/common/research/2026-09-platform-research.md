...
**LLM boundary parsing must coerce, extract, and alias — never strict-decode.** Two field failures from the same build: (1) a tool-call payload with `reps: 8` (int, schema said string) and `sets: "3"` crashed strict `Decodable` into a user-facing "could not validate" error; (2) a critic model replying with prose around its JSON (or a verdict synonym like "safe"/"flag") failed an exact-match whole-string parse. The pattern for any model-produced payload: coerce scalars both directions in a custom `init(from:)` (int↔string, double→trimmed string), extract the FIRST balanced JSON object with a brace/quote scanner instead of requiring the whole string to be JSON, and map enum vocab through an alias table with case-insensitivity. Keep a strict last resort (re-throw the typed decode) so garbage still fails visibly — tolerance is for shape, not for nonsense.

**Giant pastes kill vertical TextFields and unbounded stack layout (0x8BADF00D, third variant).** Pasting >4000 characters into `TextField(axis: .vertical)` with a `lineLimit(range)` triggers repeated full-content re-layout (TextKit sizing inside recursive HStack/VStack measurement — visible as nested `StackLayout.UnmanagedImplementation.placeChildren`/`sizeThatFits` frames in the crash stack). Three structural defenses: (1) use `TextEditor` for chat-style input — it scrolls instead of re-laying-out; (2) hard-cap input length (draft clamp + counter) so no paste is unbounded; (3) `.fixedSize(horizontal: false, vertical: true)` on long wrapping `Text` inside nested stacks to stop ideal-size propagation. Watchdog clue decoder: "process-exit" = main thread busy at termination (user swiped away mid-grind), "scene-update" = busy during backgrounding trait change — both mean "main thread was already drowning", not "exit was slow".

**LazyVStack inside ScrollView estimate-measures EVERY row when asked for ideal size (0x8BADF00D, fourth variant — use List for chat transcripts).** Crash signature: `RootGeometry.value.getter` → `LazyLayoutComputer.Engine.sizeThatFits` → `LazyStack<>.measureEstimates` ×2 → `DynamicViewList.WrappedList.applyNodes` walking item after item → per-item `lengthAndSpacing`/`lengthThatFits` → nested `StackLayout`/`_PaddingLayout` sizing, ~151 frames with reporter-detected recursion (depth 8), top frame `___chkstk_darwin`. Mechanism: a normal ScrollView proposes a bounded size, so LazyVStack only materializes visible rows — but an ideal/unbounded size query (here: the system's final transaction at process exit asking for the content's ideal extent; also triggered by some trait-change/snapshot passes) forces LazyVStack to estimate-measure EVERY row to compute total content length. A long conversation (dozens of rows × nested stacks × long wrapped Text) then burns the whole 5s termination grace on the main thread. `sorted` tie hazard discovered in the same session: Swift's `sorted` is unstable, so order-by-timestamp reads flip when a batch of messages share one `Date()` — enforce strictly increasing timestamps at write time (bump by 1ms past the batch max). Structural rule: chat transcripts (and any unbounded, expensive-row, appended-only list) belong in a `List` — UICollectionView-backed, sizes only visible cells, never runs a global estimate pass; style with hidden separators + clear backgrounds + explicit `listRowInsets` (plain List otherwise indents rows ~16pt), keep `ScrollViewReader`/`scrollTo` which work on List rows, and note `.contentMargins(_, for: .listContent)` doesn't exist on iOS — use `listRowInsets` instead.

**Three failure modes from one user session (build 12): silent error erasure, token-cap retry determinism, and unkillable background state.** (1) A custom error struct that doesn't conform to `LocalizedError` bridges to NSError as "The operation couldn't be completed. (MyApp.MyError error 1.)" — `error.localizedDescription` returns boilerplate, so every carefully-written validation message is invisible to the user AND to you during diagnosis. Always conform custom errors to `LocalizedError` (`errorDescription`), and for Decodable failures include `decodingDetail` (key + codingPath from `DecodingError.Context`) so "malformed payload" becomes "missing field 'reps' at sessions.2.exercises.0". (2) An LLM critic capped at max_tokens 512 with reasoning models: JSON truncated mid-object → balanced-brace extraction returns nil → "unparseable" → retry sends the identical request → identical truncation. Retry cannot fix a deterministic failure. Fix was threefold: raise the cap, salvage truncated JSON by scanning for the quoted key `"verdict":` (include the colon — a string VALUE is followed by comma/brace, never a colon, which distinguishes key from value), and surface the real failure reason. Also: any state machine with a "busy" flag must check the flag BEFORE rendering stale state — the retry button looked dead because the view rendered the stale failed outcome and ignored `criticRunning`. (3) Background URLSession handoff with persisted status but in-memory task tracking: if the app dies mid-handoff, the restored task's completion hits a missing tracking entry and is silently dropped, leaving a "replying in the background..." banner forever. Fixes: stamp `task.taskDescription = turnID` (survives relaunch), inject a registry lookup to recover restored tasks, and run a staleness sweep (anything handed off >10 min is failed with an unacknowledged error that surfaces the existing retry notice) at launch + foreground. General rule: any persisted "in-flight" state needs a reconciliation pass at startup — the completion that would have cleared it may never come.

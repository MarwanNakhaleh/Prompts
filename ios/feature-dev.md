...
- For common free-text inputs (phone numbers, postal codes, dates, credit card numbers), **accept multiple formats and normalize internally** rather than rejecting valid data over cosmetic differences. Strip spaces, hyphens, and parentheses from phone numbers. Accept dates in whatever format the user typed and parse them. Don't force a specific format unless the format itself is the meaningful data (e.g., a product serial number). Always validate the semantic value (valid date range, valid card length) after normalizing.
...
- Design tappable elements to be at least **44×44 pt** — Apple's minimum recommended hit target. Add adequate spacing between adjacent targets; placing a destructive action (delete, remove) immediately beside a confirm action invites accidental activation even at the correct size. UIKit and SwiftUI will not automatically expand a control's hit area beyond its rendered bounds.
- **Minimize required typing**: prefer pickers, steppers, segmented controls, and selection lists over free-text entry where the value comes from a bounded set. Where text entry is unavoidable, set the most specific keyboard type (`.numberPad`, `.emailAddress`, `.phonePad`) and populate `.textContentType` for autofill. Pre-fill values you can reasonably infer from context (current locale, previously entered data, device region).

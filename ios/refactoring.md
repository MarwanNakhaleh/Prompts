...
- Flag **methods too complex to decompose without shared intermediate state**: when a long method performs multiple related sub-steps that pass data between them via a growing list of local variables, extract the entire method into a dedicated struct or class. Its parameters become initializer arguments; its local variables become stored properties; each sub-step becomes a separate named method. This **Method Object** move differs from "Introduce Parameter Object" (which groups only parameters) — it relocates the whole computation into its own scope so the sub-steps can share state through the type's properties rather than a bloated local-variable stack, and each sub-step becomes independently testable.
...
- Flag huge `switch`/`if-else` ladders over a type that could become polymorphism or an enum with associated values. When naming the refactoring move, give it a pattern target: conditional branching on type to select behavior converges on **Strategy** (inject the varying behavior behind a protocol); branching driven by state transitions converges on **State** (each state as its own type conforming to a protocol); a notification/observer structure converges on **Observer**; a type that needs responsibilities layered on dynamically converges on **Decorator**. Naming the pattern target shares vocabulary with reviewers and gives the refactoring a concrete blueprint — a "Replace Conditional with Polymorphism" move without a target is harder to scope, review, and finish.
...
- **Write fitness functions to enforce architectural boundaries in CI.** A fitness function is an automated test that fails when a forbidden cross-boundary dependency is introduced — not a rule enforced by review, but a check that runs every build and cannot be skipped under time pressure. For iOS/Swift: write an XCTest (or a Swift script run from a build phase) that asserts no Swift package in the `Sales` product imports any target in the `Warehouse` product; use `swift package show-dependencies` output or parse import statements with a grep to derive the dependency graph. Where formal Swift packages are not yet set up, a SwiftLint `prohibited_import` rule or a shell script that greps for forbidden `import` statements and exits non-zero achieves the same intent. The cost is writing the assertion once; the return is automated boundary enforcement for the life of the codebase — a CI gate that gives the same refactoring freedom as a real package boundary without requiring the full package split first.
